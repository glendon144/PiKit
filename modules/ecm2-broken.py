...
class ECMEngine:
    ...
    def _compute_once(self, now: Optional[float] = None) -> None:
        ...
        # Normalize BPM into 0–1
        bpm_norm = min(max((bpm - BPM_MIN) / (BPM_MAX - BPM_MIN), 0.0), 1.0)

        # Combine: speed + acceleration + fragmentation
        load = (
            0.45 * bpm_norm +
...
_engine = ECMEngine(window_seconds=WINDOW_SECONDS_DEFAULT, db_path=DEFAULT_DB_PATH)
_engine.start()
...
def put_override(body: OverrideIn, response: Response):
    for k, v in _cors_headers("http://localhost").items():
        response.headers[k] = v
    _engine.set_override(body.factor)
    return _engine.now()
...
def post_label(body: LabelIn, response: Response):
    for k, v in _cors_headers("http://localhost").items():
        response.headers[k] = v
    _engine.label_slice(body.start_ts, body.end_ts, body.label)
